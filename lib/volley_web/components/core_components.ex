defmodule VolleyWeb.CoreComponents do
  @moduledoc """
  ...
  attr :variant, :string,
    values: ~w(primary create secondary neutral delete ghost scorer-action save cancel)
  ...
  def button(%{rest: rest} = assigns) do
    variants = %{
      ...
      "create" => "btn-success dark:btn-soft",
      "delete" => "btn-error dark:btn-soft",
      "ghost" => "btn-ghost",
      "scorer-action" => "btn-block btn-md p-4",
      "save" => "btn-success not-dark:btn-outline",
      "cancel" => "dark:btn-neutral not-dark:btn-outline",
      nil => ""
    }

    assigns =
      assign(assigns, :btn_class, ["btn", Map.fetch!(variants, assigns[:variant])])

    if rest[:href] || rest[:navigate] || rest[:patch] do
      ~H"""
      <.link class={[@btn_class, @class]} {@rest}>
        {render_slot(@inner_block)}
      </.link>
      """
    else
      ~H"""
      <button class={[@btn_class, @class]} {@rest}>
        {render_slot(@inner_block)}
      </button>
      """
    end
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(VolleyWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(VolleyWeb.Gettext, "errors", msg, opts)
    end
  end
