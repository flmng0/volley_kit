defmodule VolleyWeb.UserLive.Login do
  ...
  defp render_intro(assigns) do
    ~H"""
    <.button phx-click="select_view" phx-value-view="magic" variant="neutral" class="w-full">
      Log in with Email and Magic Link
    </.button>
    <div class="divider">or</div>
    <.button phx-click="select_view" phx-value-view="password" variant="neutral" class="w-full">
  ...
  defp render_password_form(assigns) do
    ...
    <.button variant="neutral" class="w-full" phx-click="select_view" phx-value-view="magic">
  ...
  def render(assigns) do
    ...
    <Layouts.app flash={@flash}>
      <div class="mx-auto max-w-sm space-y-4">
        ...
        <.render_intro :if={@view == nil} />
        <.render_magic_form :if={@view == "magic"} form={@form} current_scope={@current_scope} />
        ...
        />
        <.render_magic_submitted :if={@view == "magic_submitted"} />
      </div>
    </Layouts.app>
    """
  end
  ...
  def mount(_params, _session, socket) do
    ...
    {:ok, assign(socket, form: form, trigger_submit: false, view: nil)}
  end
  ...
  def handle_event("submit_magic", %{"user" => %{"email" => email}}, socket) do
    ...
    {:noreply, assign(socket, :view, "magic_submitted")}
  end
